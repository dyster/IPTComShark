            ...
            for (int i = 0; i < itemcount; i++)
            {
                ...
                switch (itemtype)
                {
                    ...
                    case ENIP_CPF_Types.CPF_ITEM_CONNECTED_DATA:
                        ...
                        else if (traveller.Source[0] == 192 && traveller.Source[1] == 168 && traveller.Source[2] == 1 && traveller.Source[3] == 15)
                        {
                            // from duagon
                            ...
                            if (traveller.Destination[0] == 192 && traveller.Destination[1] == 168 && traveller.Destination[2] == 1 && traveller.Destination[3] == 101)
                            {
                                ...
                                var pd = CIP.GWtoDMI.Parse(data, false, pos * 8);
                                parse.ParsedData.Add(pd);
                                parse.BackLinkIdentifier = "122";
                                parse.AutoGenerateDeltaFields = true;
                                parse.Name = "GW->DMI-L";
                            }
                            else if (traveller.Destination[0] == 192 && traveller.Destination[1] == 168 && traveller.Destination[2] == 1 && traveller.Destination[3] == 102)
                            {
                                ...
                                var pd = CIP.GWtoDMI.Parse(data, false, pos * 8);
                                parse.ParsedData.Add(pd);
                                parse.BackLinkIdentifier = "222";
                                parse.AutoGenerateDeltaFields = true;
                                parse.Name = "GW->DMI-R";
                            }
                            else if (traveller.Destination[0] == 192 && traveller.Destination[1] == 168 && traveller.Destination[2] == 1 && traveller.Destination[3] == 21)
                            {
                                // to CPUA
                                if (data.Length > pos) //TODO why do I have to check this on this data set, but not on the others??
                                {
                                    var cipioheader = CrackUInt32(data, pos);
                                    pos += 4;
                                    pds.ParsedFields.Add(ParsedField.Create("32-bit header", cipioheader));

                                    var pd = CIP.GWtoEVC1.Parse(data, false, pos * 8);
                                    parse.ParsedData.Add(pd);
                                    parse.BackLinkIdentifier = "111";
                                    parse.AutoGenerateDeltaFields = true;
                                }
                            }
                            else if (traveller.Destination[0] == 192 && traveller.Destination[1] == 168 && traveller.Destination[2] == 1 && traveller.Destination[3] == 23)
                            {
                                // to CPUB
                                if (data.Length > pos) //TODO why do I have to check this on this data set, but not on the others??
                                {
                                    ...
                                    var pd = CIP.GWtoEVC2.Parse(data, false, pos * 8);
                                    parse.ParsedData.Add(pd);
                                    parse.BackLinkIdentifier = "124";
                                    parse.AutoGenerateDeltaFields = true;
                                }
                            }
                        }
                    ...
                    case ENIP_CPF_Types.CPF_ITEM_UNCONNECTED_DATA:
                        ...
                        else
                        {
                            ...
                            if (service == 0x02)
                            {
                                ...
                                if (traveller.Source[0] == 192 && traveller.Source[1] == 168 && traveller.Source[2] == 1 && traveller.Source[3] == 30 &&
                                    traveller.Destination[0] == 192 && traveller.Destination[1] == 168 && traveller.Destination[2] == 1 && traveller.Destination[3] == 15)
                                {
                                    parse.Name = "TPWS->GW";
                                    parse.AutoGenerateDeltaFields = true;
                                    parse.BackLinkIdentifier = "201";
                                    var ds = CIP.TPWStoGW.Parse(buffer);
                                    parse.ParsedData.Add(ds);
                                }
                                else
                                {
