            ...
            if (topPacket.PayloadPacket is IPv4Packet)
            {
                ...
                if (ipv4.Protocol == PacketDotNet.ProtocolType.Udp)
                {
                    ...
                    if (packet.Protocol == ProtocolType.IPTWP)
                    {
                        ...
                        {
                            var parse = new Parse() { ParsedData = new List<ParsedDataSet>() { parsedDataSet }, BackLinkIdentifier = packet.IPTWPPacket.Comid.ToString(), AutoGenerateDeltaFields = true, Name = parsedDataSet.Name};
